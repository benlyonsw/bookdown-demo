...
yearly_modified$parkwide <- yearly_modified$UnitCode
```

```{r}
yearly_modified <- yearly_modified %>% mutate(parkwide = ifelse(parkwide == "Parkwide", "Parkwide", "Parks"))
```
...
###### I tried using a 'for' loop to accomplish this, but R disagreed.

```{r}
wide_data_percent = subset(wide_data_percent, select = -(Parkwide))
```

```{r}
narrow_data_percent <- wide_data_percent %>%
  pivot_longer(cols = -Year,
               names_to = "Park",
               values_to = "Percent")
```
...
```{r}
joined_data <- joined_data %>% mutate(UnitCode.y = "Parkwide")
```
...
joined_data <- joined_data %>%
  ...
  rename(NationwideVisitors = RecreationVisitors.y)
```


```{r}
joined_yearly <- joined_data %>%
  group_by(UnitCode, Year) %>% 
  summarize(NatVisitors = sum(NationwideVisitors), RecVisitors = sum(RecreationVisitors))
```

```{r}
joined_yearly$percentage <- (joined_yearly$RecVisitors/joined_yearly$NatVisitors)*100
```
...
```{r}
colMeans(percentages)
```

On average, Great Smokey Mountains National Park has the greatest visitation (3.5% of all visits), while Acadia National Park has the least (0.93% of all visits).