...
```{r}
EightiesRaw <- (httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980"))
EightiesUnpacked <- (httr::content(EightiesRaw, as = "text", encoding = "UTF-8"))
EightiesFinal <- (jsonlite::fromJSON(EightiesUnpacked))

NinetiesRaw <- (httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999"))
NinetiesUnpacked <- (httr::content(NinetiesRaw, as = "text", encoding = "UTF-8"))
NinetiesFinal <- (jsonlite::fromJSON(NinetiesUnpacked))

TwentyTensRaw <- (httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018"))
TwentyTensUnpacked <- (httr::content(TwentyTensRaw, as = "text", encoding = "UTF-8"))
TwentyTensFinal <- (jsonlite::fromJSON(TwentyTensUnpacked))
```
...
```{r}
RomoRaw <- (httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=1&startYear=2010&endMonth=12&endYear=2021"))

RomoUnpacked <- (httr::content(RomoRaw, as = "text", encoding = "UTF-8"))

RomoFinal <- (jsonlite::fromJSON(RomoUnpacked))
...
unit_visitation <- function(unitCodes, startMonth, startYear, endMonth, endYear){
  raw_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=",unitCodes, "&startMonth=",startMonth,"&startYear=",startYear,"&endMonth=", endMonth, "&endYear=",endYear))
...
```{r}
RomoData <- unit_visitation("ROMO", 01, 1990, 12, 2021)
EverData <- unit_visitation("EVER", 01, 1990, 12, 2021)
ThroData <- unit_visitation("THRO", 01, 1990, 12, 2021)
```
...
```{r}
unit_visitation2 <- function(unitCodes, startMonth = "01", startYear, endMonth = "12", endYear){
  raw_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=",unitCodes, "&startMonth=",startMonth,"&startYear=",startYear,"&endMonth=", endMonth, "&endYear=",endYear))
...
```{r}
ROMONew <- unit_visitation2("ROMO", startYear = 1980, endYear = 2021)
EVERNew <- unit_visitation2("EVER", startYear = 1980, endYear = 2021)
THRONew <- unit_visitation2("THRO", startYear = 1980, endYear = 2021)
```
...
output_floop2 <- vector("list", length = length(parks))
```
...
multi_parks <- bind_rows(output_map_parks)
```
